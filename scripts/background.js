...
function reload_i18n_messages(language, callback) {
  ...
  callback && callback();
  // need sandbox to use eval: http://developer.chrome.com/stable/extensions/sandboxingEval.html
...
var friendships = {
  ...
  destroy: function (user_id, screen_name, callback) { //取消更随某人
    ...
    tapi.friendships_destroy(params, function (user_info, textStatus, statuCode) {
      if (user_info === true || user_info.id) {
        ...
        window.showMsg(_u.i18n("msg_f_destroy_success").format({name: screen_name}));
      } else {
        user_info = null;
      }
      if (callback) { 
  ...
  fetch_friends: function (user_uniquekey, callback, context) {
    var user = null;
    if (user_uniquekey) {
      user = window.getUserByUniqueKey(user_uniquekey, 'all');
    }
    if (!user) {
      user = getUser();
    }
...
function checkTimeline(t, user_uniqueKey) {
  ...
  var params = { user: c_user, count: window.PAGE_SIZE };
  var last_id = null;
...
try {
  ...
  log('Not Support Audio: ' + err.message);
}
    ...
    '<div class="item">' +
        ...
        '<div class="info"><span class="username">{{user.screen_name}}</span><br/>' + 
            '<span class="unreads">' + 
                ...
                _u.i18n("abb_friends_timeline") + '</span> &nbsp;&nbsp; <span id="unr_mentions"><span>{{unreads.mentions}}</span>@</span> <br/>' + 
                '<span id="unr_comments_timeline"><span>{{unreads.comments_timeline}}</span>' +
                _u.i18n("abb_comment") + '</span> &nbsp;&nbsp; <span id="unr_direct_messages"><span>{{unreads.direct_messages}}</span>' +
                _u.i18n("abb_direct_message") + '</span> ' + 
            '</span>' + 
        '</div>' + 
    '</div>' + 
  ...
  start: function (getFirst) {
    ...
    for (var j = 0, jlen = userList.length; j < jlen; j++) {
      ...
      if (user.oauth_refresh_token && user.oauth_expires_in) {
        ...
        refTime = (user.oauth_expires_in / 2) * 1000;
